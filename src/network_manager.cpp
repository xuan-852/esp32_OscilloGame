...
// --- 无线手柄状态 ---
static GamepadData latest_gamepad_data = {};                 // 最新手柄数据
static unsigned long last_gamepad_time = 0;                  // 最后收到手柄数据的时间
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    // --- 无线手柄数据处理 (来自硬编码从机MAC) ---
    if (len == sizeof(GamepadData) && memcmp(mac, GAMEPAD_SLAVE_MAC, 6) == 0) {
        memcpy((void*)&latest_gamepad_data, incomingData, sizeof(GamepadData));
        last_gamepad_time = millis();
        // 每秒打印一次手柄状态
        static unsigned long last_gp_print = 0;
        if (millis() - last_gp_print > 1000) {
            last_gp_print = millis();
            Serial.printf("[GAMEPAD] J1X:%u J1Y:%u A:%u B:%u ENC:%d DIR:%u\n",
                latest_gamepad_data.joy1X, latest_gamepad_data.joy1Y,
                latest_gamepad_data.btnA, latest_gamepad_data.btnB,
                latest_gamepad_data.encDelta, latest_gamepad_data.dirPad);
        }
        return;
    }
...
void Network_Manager::init() {
    ...
    // --- 注册无线手柄从机为 ESP-NOW 对等设备 ---
    Serial.printf("[GAMEPAD] My MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
        my_mac[0], my_mac[1], my_mac[2], my_mac[3], my_mac[4], my_mac[5]);
    Serial.printf("[GAMEPAD] Slave MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
        GAMEPAD_SLAVE_MAC[0], GAMEPAD_SLAVE_MAC[1], GAMEPAD_SLAVE_MAC[2],
        GAMEPAD_SLAVE_MAC[3], GAMEPAD_SLAVE_MAC[4], GAMEPAD_SLAVE_MAC[5]);
    
    if (!esp_now_is_peer_exist(GAMEPAD_SLAVE_MAC)) {
        esp_now_peer_info_t peerInfo;
        memset(&peerInfo, 0, sizeof(peerInfo));
        memcpy(peerInfo.peer_addr, GAMEPAD_SLAVE_MAC, 6);
        peerInfo.channel = 0;
        peerInfo.encrypt = false;
        peerInfo.ifidx = WIFI_IF_AP;
        if (esp_now_add_peer(&peerInfo) == ESP_OK) {
            Serial.println("Gamepad slave peer registered");
        }
    }
}
...
bool Network_Manager::isGamepadConnected() {
    return (millis() - last_gamepad_time) < GAMEPAD_TIMEOUT_MS;
}

bool Network_Manager::getGamepadData(GamepadData* out) {
    if (!isGamepadConnected()) return false;
    memcpy(out, (const void*)&latest_gamepad_data, sizeof(GamepadData));
    return true;
}